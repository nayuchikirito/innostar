...
<script type="text/javascript">
  $(function(){
    var table = $('#client-requests-table').DataTable({
      ...
      bServerSide: false,
      sServerMethod: "GET",
      'ajax': '/admin/get-client-requests',
      searching: true, 
      paging: true, 
      filtering:false, 
      bInfo: true,
            ...
            bootbox.confirm({
              ...
              callback: function (result) {
                 if(result){
                  ...
                  $.ajax({
                  url:'/admin/client_requests_approve/'+that.dataset.id,
                  type: 'put',
                  data: {_method: 'put', _token :token},
                  success:function(result){
                    $("#client-requests-table").DataTable().ajax.url( '/admin/get-client-requests' ).load();
                    swal({
                        title: result.msg,
                        icon: "success"
                      });
            ...
            bootbox.confirm({
              ...
              callback: function (result) {
                 if(result){
                  var token = '{{csrf_token()}}'; 
                  $.ajax({
                  url:'/admin/client_requests_decline/'+that.dataset.id,
                  type: 'put',
                  data: {_method: 'put', _token :token},
                  success:function(result){
                    $("#client-requests-table").DataTable().ajax.url( '/admin/get-client-requests' ).load();
